...
class MLService:
    def __init__(self):
        ...
        if YOLO:
            try:
                # Load YOLOv8 nano for CPU efficiency
                self.model = YOLO('yolov8n.pt')
                self.active = True
                logger.info("YOLOv8n model loaded successfully for CPU inference.")
    ...
    def process_frame(self, camera_id: int, frame):
        ...
        try:
            ...
            with self.inference_lock:
                results = self.model.track(frame, persist=True, device='cpu', verbose=False, classes=list(self.target_classes.keys()))
            ...
            for track_id, cls_id, conf, xyxy in zip(track_ids, cls_ids, confs, xyxys):
                ...
                # Check if new object or dwelling
                if track_id not in self.object_history[camera_id]:
                    # New object detected -> Routine Alert
                    self.object_history[camera_id][track_id] = {"first_seen": current_time, "last_seen": current_time}
                    alerts.append({
